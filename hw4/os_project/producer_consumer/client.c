...
void downloader(int target_id) {
    ...
    while (1) {
        ...
        // int sync_p = semop(target_id, &p, 1);
        int i;
        for (i = 0; i < BUFNUM; i++) {
            if (semop(sem_id[i], &p, 1) < 0) {
                /* error handling code */
                perror("wait error");
                exit(1);
            }
        ...
        /* TODO: sync downloader and writer */
        // int sync_v = semop(target_id, &v, 1);
        for (i = 0; i < BUFNUM; i++) {
            if (semop(sem_id[i], &v, 1) < 0) {
                /* error handling code */
                perror("signal error");
                exit(1);
            }
...
void IPC_init() {
    ...
    shm_id =
        shmget(IPC_PRIVATE, sizeof(struct Buffer) * BUFNUM, IPC_CREAT | 0600);
    ...
    int i;
    for (i = 0; i < BUFNUM; i++) {
        sem_id[i] = semget(IPC_PRIVATE, 256, IPC_CREAT | 0600);
        if (sem_id[i] < 0) {
            /* error handling code */
            perror("semget");
            exit(1);
        ...
        sem_union.val = 1;
        if (semctl(sem_id[i], 0, SETVAL, sem_union) ==
            -1) {  // SETVAL is a macro to specify that you're setting the value
                   // of
